#!/usr/bin/python3
#author: Sari Sabban
#edited by Rob de Haas

import os
import re
import sys
import bs4 #pip install, and pip install lxml
import time
import datetime
import requests
import argparse
...
def Fragments(filename):
	'''
	Submits the pose to the Robetta server (http://www.old.robetta.org) for
	...
	web = requests.get('http://old.robetta.org/fragmentsubmit.jsp')
	payload = {	'UserName':          '',
				'Email':             'dehaas.robbert@gmail.com',
	...
	response = session.post('http://old.robetta.org/fragmentsubmit.jsp', data=payload, files=dict(foo='bar'))
	for line in response:
		line = line.decode()
		if re.search('<a href="(fragmentqueue.jsp\?id=[0-9].*)">', line):
			JobID = re.findall('<a href="(fragmentqueue.jsp\?id=[0-9].*)">', line)
	JobURL = 'http://old.robetta.org/' + JobID[0]
	ID = JobID[0].split('=')
	URL = 'http://old.robetta.org/fragmentqueue.jsp'
...
'''.format('-'*57))
	...
	for item in info:
		...
		if status == 'Queued':
			...
			TBL = '\u001b[33m|{} |{}| {}\t | {}\u001b[0m'.format(j, s, l, n)
		print(TBL)
	print('Fragments submitted to Robetta server [http://old.robetta.org/fragmentqueue.jsp]')
	...
	os.system('wget -q http://old.robetta.org/downloads/fragments/'+str(ID[1])+'/aat000_03_05.200_v1_3')
	print('Downloading 9mer fragment file ...')
	os.system('wget -q http://old.robetta.org/downloads/fragments/'+str(ID[1])+'/aat000_09_05.200_v1_3')
	#print('Downloading PSIPRED file ...')
	#os.system('wget -q http://old.robetta.org/downloads/fragments/'+str(ID[1])+'/t000_.psipred_ss2')
